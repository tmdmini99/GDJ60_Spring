...
$("#btn").click(function(){
    ...
    form.append("contents",$("#inp3").val());//<form><input type="text" name="contents" value="dfds"> </form>
    form.append("bookNumber",$("#btn").attr('data-book-bookNumber'));//<form><input type="text" name="contents" value="dfds"> <input type="text" name="bookNumber" value="dfds"></form>
...
$("#comment").on("click",".page-link",function(e){
...
// });
//delete
$("#comment").on("click",".dt",function(e){
         ...
         let res=fetch("../bankBookComment/delete", {
            ...
            body:"num="+$(this).attr('data-delete')
            //응답객체에서 Data 추출
...
$("#comment").on("click",".up",function(e){
    let num =$(this).attr("data-update");
    let contents =$("#contents"+num);
    let contentsTextArea=$("#contents");
    contentsTextArea.val($("#contents"+num).text());
    $("#contentsConfirm").attr("data-update",num);
...
$("#contentsConfirm").click(function(){
    fetch("../bankBookComment/update",{
        method:"POST",
        headers:{"Content-type": "application/x-www-form-urlencoded"},
        body:"num="+$(this).attr("data-update")+"&contents="+$("#contents").val()
    }).then((response)=>response.text()).then((res)=>{
                            if(res.trim()>0){
                                alert("수정 성공");
                                getList(1);
